...
export function renderLoading(display: HTMLElement): void {
  display.innerHTML = `
    <div class="loader">
...
export function renderError(display: HTMLElement, err: ApiError): void {
  display.innerHTML = `
    <div class="flex items-center flex-col text-center mt-20">
      ...
      <p class="text-sm xs:text-lg font-normal text-neutral-500 mt-5">
        ${err.message ?? ""} ${err.resolution ?? ""}
      </p>
    </div>
  `;
}

export function renderWord(data: WordData, display: HTMLElement): void {
